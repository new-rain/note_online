        ...
        <div class="friendList mgt_20">
          <el-row>
            <el-col :span="6" class="left">
              ...
              <br>
              <el-button icon="el-icon-jxy-iconhuxiangguanzhu" class="wd_90" type="primary" plain
                         @click="chooseWhat(2)">互相关注
              </el-button>
              <br>
            ...
            <el-col :span="18" class="right ta_l">
              ...
              <div v-for="friend in show">
                <el-row class="fri mgb_20">
                  <el-col :span="3">
                    <img @click="see(friend.no)" :src="friend.headImg" alt="好友头像"/>
                  </el-col>
                  <el-col :span="18">
  ...
  methods: {
    ...
    selectEachOther(no) {
      this.$axios.get("/selectEachOther", {
        ...
        this.each = res.data;
        for (let i = 0; i < this.each.length; i++) {
          let headUrl = this.each[i].headUrl;
          this.$axios({
            method: 'get',
            url: '/getUserHead?headUrl=' + headUrl,
            responseType: 'arraybuffer' // 最为关键
          }).then(response => {
            this.each[i].headImg = 'data:image/jpeg;base64,' + this.arrayBufferToBase64(response.data)
          })
        }
    ...
    getUserHead(headUrl) {
      this.$axios({
        ...
        this.h = 'data:image/jpeg;base64,' + this.arrayBufferToBase64(response.data)
      })
    },
    ...
    selectCareMe(no) {
      this.$axios.get("/selectCareMe", {
        ...
        for (let i = 0; i < this.careMe.length; i++) {
          let headUrl = this.careMe[i].headUrl;
          this.$axios({
            method: 'get',
            url: '/getUserHead?headUrl=' + headUrl,
            responseType: 'arraybuffer' // 最为关键
          }).then(response => {
            this.careMe[i].headImg = 'data:image/jpeg;base64,' + this.arrayBufferToBase64(response.data)
          })
        }
    ...
    selectMyCare(no) {
      this.$axios.get("/selectMyCare", {
        ...
        for (let i = 0; i < this.Mycare.length; i++) {
          let headUrl = this.Mycare[i].headUrl;
          this.$axios({
            method: 'get',
            url: '/getUserHead?headUrl=' + headUrl,
            responseType: 'arraybuffer' // 最为关键
          }).then(response => {
            this.Mycare[i].headImg = 'data:image/jpeg;base64,' + this.arrayBufferToBase64(response.data)
          })
        }
