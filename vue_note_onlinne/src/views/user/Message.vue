  ...
  methods: {
    ...
    readAll() {
      ...
      this.$axios.put("/readAll", params).then(res => {
        if (res.data) {
          this.havaNew[this.type] = false;
          this.getMessages(no, type, 1);
        }
      }).catch(error => {
        console.log(error)
      })
    ...
    goCheck(msg) {
      ...
      this.$axios.put("/checkMsg?no=" + msg.mno).then(res => {
        if (res.data) {
          this.check = true;
          this.getMessages(this.no, this.type, this.page);
          this.checkHaveNew(this.no);
        }
      }).catch(error => {
      ...
      this.$axios.delete("/deleteMsg?no=" + no).then(res => {
        if (res.data) {
          ...
          if (this.messages.length >= 0) {
            if (this.page - 1 > 0) {
              this.getMessages(this.no, this.type, this.page - 1);
            }
          }
        } else {
